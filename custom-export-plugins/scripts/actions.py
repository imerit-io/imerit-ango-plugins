...
def test(**data):
    ...
    # if you want to filter by batch, you can get the batch from the config and filter the json export
    try:
        batch = config["batch"]
        ...
        if batch != "":
            json_export = [
                asset for asset in _temp_unfiltered_json if batch in asset["batches"]
            ]
        elif batch == "":
            json_export = _temp_unfiltered_json
    except KeyError:
